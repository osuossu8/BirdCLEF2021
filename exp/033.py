...
def cutmix(data, targets, secondary_targets, alpha):
    ...
    new_secondary_targets = [secondary_targets, shuffled_secondary_targets, lam]
    return data, new_targets, new_secondary_targets
...
def mixup(data, targets, secondary_targets, alpha):
    ...
    new_targets = [targets, shuffled_targets, lam]
    new_secondary_targets = [secondary_targets, shuffled_secondary_targets, lam]
    return new_data, new_targets, new_secondary_targets

def cutmix_criterion(preds, new_targets, new_secondary_targets):
    targets1, targets2, lam = new_targets[0], new_targets[1], new_targets[2]
    secondary_targets1, secondary_targets2, lam = new_secondary_targets[0], new_secondary_targets[1], new_secondary_targets[2]
    criterion = SedScaledPosNegFocalLoss()
    return lam * criterion(preds["clipwise_output"], (targets1, secondary_targets1)) + (1 - lam) * criterion(preds["clipwise_output"], (targets2, secondary_targets2))

def mixup_criterion(preds, new_targets, new_secondary_targets):
    targets1, targets2, lam = new_targets[0], new_targets[1], new_targets[2]
    secondary_targets1, secondary_targets2, lam = new_secondary_targets[0], new_secondary_targets[1], new_secondary_targets[2]
    criterion = SedScaledPosNegFocalLoss()
    return lam * criterion(preds["clipwise_output"], (targets1, secondary_targets1)) + (1 - lam) * criterion(preds["clipwise_output"], (targets2, secondary_targets2))
...
def train_mixup_cutmix_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        if np.random.rand()<0.5:
            inputs, new_targets, new_secondary_targets = mixup(inputs, targets, secondary_targets, 0.4)
            outputs = model(inputs)
            loss = mixup_criterion(outputs, new_targets, new_secondary_targets) 
        else:
            inputs, new_targets, new_secondary_targets = cutmix(inputs, targets, secondary_targets, 0.4)
            outputs = model(inputs)
            loss = cutmix_criterion(outputs, new_targets, new_secondary_targets)
        if CFG.apex:
            with amp.scale_loss(loss, optimizer) as scaled_loss:
                scaled_loss.backward()
        else:
