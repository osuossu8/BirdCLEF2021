...
class CFG:
    ...
    # Model #
    ######################
    base_model_name = "tf_efficientnet_b0_ns" # "resnest50d_1s4x24d"
    pooling = "max"
    pretrained = True
    num_classes = 398
    in_channels = 1
...
def cutmix_criterion(preds, new_targets):
    ...
    return (lam * criterion(preds, targets1) + (1 - lam) * criterion(preds, targets2)) * 0.8 + (lam * criterion(preds, secondary_targets1) + (1 - lam) * criterion(preds, secondary_targets2)) * 0.2
