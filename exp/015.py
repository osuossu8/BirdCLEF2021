...
class CFG:
    ...
    # Model #
    ######################
    base_model_name = "tf_efficientnet_b0_ns" # "tf_efficientnet_b1_ns"
    pooling = "max"
    pretrained = True
    num_classes = 398
    in_channels = 1
...
new_train = pd.read_csv('inputs/image_folds_with_weak_labeled_by_013.csv')
new_train['pred'] = new_train['pred'].fillna('')
...
for fold in range(5):
    ...
    train_fold_s['primary_label'] = train_fold_s['secondary_labels']
    del train_fold_s['secondary_labels'], train_fold_s['rating'], train_fold_s['pred']

    train_fold_m = train_fold.query('len_label>1')
    del train_fold_m['secondary_labels'], train_fold_m['rating'], train_fold_m['pred']
